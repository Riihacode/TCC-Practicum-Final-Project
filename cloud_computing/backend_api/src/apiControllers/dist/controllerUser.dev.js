...
function uploadProfilePic(req, res) {
  var user_id, contentType, busboy, fileBuffer, filename, fileReceived, uploadError, parseForm, user, sanitized, finalFilename, uploadDir, savePath, fileUrl;
  return regeneratorRuntime.async(function uploadProfilePic$(_context7) {
    while (1) {
      switch (_context7.prev = _context7.next) {
        case 0:
          user_id = req.params.user_id; // ✅ Validasi awal: pastikan content-type ada dan multipart
        ...
        case 25:
          ...
          uploadDir = _path["default"].join(process.cwd(), "public", "upload", "users", user_id, "uploadedUserPhotoProfile");

          _fs["default"].mkdirSync(uploadDir, {
            recursive: true
          });

          savePath = _path["default"].join(uploadDir, finalFilename);
          fileUrl = "/upload/users/".concat(user_id, "/uploadedUserPhotoProfile/").concat(finalFilename);

          _fs["default"].writeFileSync(savePath, Buffer.concat(fileBuffer));
...
function deleteProfilePic(req, res) {
  var user_id, user, profilePicPath;
  return regeneratorRuntime.async(function deleteProfilePic$(_context9) {
    while (1) {
      switch (_context9.prev = _context9.next) {
        ...
        case 7:
          profilePicPath = _path["default"].join(process.cwd(), "public", user.profile_pic);

          if (_fs["default"].existsSync(profilePicPath)) {
            _fs["default"].unlinkSync(profilePicPath);

            console.log("[DELETE-PROFILE] File deleted: ".concat(profilePicPath));
          }
        ...
        case 15:
          ...
          console.error("[DELETE-PROFILE-ERROR] ".concat(_context9.t0.message));
          res.status(500).json({
...
function updateProfilePic(req, res) {
  var user_id, contentType, busboy, fileBuffer, filename, fileReceived, uploadError, parseForm, user, oldPath, sanitized, finalFilename, uploadDir, savePath, newProfilePicUrl;
  return regeneratorRuntime.async(function updateProfilePic$(_context10) {
    while (1) {
      switch (_context10.prev = _context10.next) {
        case 0:
          user_id = req.params.user_id; // ✅ Validasi Content-Type terlebih dahulu
        ...
        case 23:
          ...
          if (user.profile_pic) {
            oldPath = _path["default"].join(process.cwd(), "public", user.profile_pic.replace(/^\/+/, ""));
            if (_fs["default"].existsSync(oldPath)) _fs["default"].unlinkSync(oldPath);
          }
          ...
          savePath = _path["default"].join(uploadDir, finalFilename);
          newProfilePicUrl = "/upload/users/".concat(user_id, "/uploadedUserPhotoProfile/").concat(finalFilename);
